...
- **Function entrypoint signature — MUST be `def run(inp: dict[str, Any]) -> dict[str, Any]`.** The executor loads the source file, reads `getattr(module, "run", None)`, and calls it as `entry(args)` where `args` is the rendered `config.arguments` dict as a single positional. Destructure by argument name inside the function body — `summary = inp["summary"]`, etc. A signature that names its parameter after the argument key (e.g. `def run(summary: dict)`) is silently miscalled to `run({"summary": {...}})`: Python binds the whole wrapper dict to `summary`, and any `.get("status", "")`-style defaulting then returns empty strings that pass schema validation and produce semantically-empty findings. This class of bug does not fail loudly — every downstream node succeeds, every schema check passes, and the emit node ships a shape-valid finding with all fields blank. Do NOT use signatures like `def run(summary, page)` or `def run(*, summary)` — Python's binding rules produce a different silent failure for each shape. The single-`inp`-dict form is the only one the executor calls correctly.
- **Fail loud on missing keys.** In function bodies, missing keys in `inp` MUST raise `KeyError`/`TypeError` — do NOT default to `""` / `[]` / `0`. Silent defaults let a wrong-arg-shape call produce schema-valid empty output that reviewers cannot distinguish from a real "everything healthy" result. Use `inp["argName"]`, not `inp.get("argName", "")`.
...
10. **Function-entrypoint signature** — every `kind: function` node's `source` file MUST define `def run(inp: dict[str, Any]) -> dict[str, Any]` with exactly one positional dict parameter named `inp` (or an equivalent single-positional-dict shape). Any other signature is silently miscalled by the executor. Grep the emitted `functions/*.py`: they must all match `^def run(` followed by exactly one parameter binding a dict. Reject `def run(<arg-name>: dict)` shapes even when the arg-name matches the sole `config.arguments` key — this is exactly the miscall pattern this rule exists to catch.
11. **No default-to-empty in function bodies** — function nodes MUST NOT default missing `inp` keys to empty strings, empty lists, or zeros. `inp["x"]` is required; `inp.get("x", "")` is forbidden. A wrong-shape `inp` must fail the node (KeyError/TypeError), not produce shape-valid empty output that a downstream emit node happily ships.
